...
let promo__menu = document.querySelector('.promo__menu-list ul')
// movieDB.movies.sort()
...
promo__menu.innerHTML = ''
promo__interactive.innerHTML = ''

for (let item of movies) {
  let xamro = movies.filter(el => el !== item.Title)
  console.log(xamro);
  // movies = movies.filter(el => el !== item.Title)
  let promo__interactive_item = document.createElement('li')
  let promo__menu_li = document.createElement('li')
  promo__menu_li.classList.add('promo__menu-item')
  promo__menu.append(promo__menu_li)
  promo__menu_li.innerHTML = item.Genre
